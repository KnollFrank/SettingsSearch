...
public class SubtreeReplacerTest {
    ...
    // Vertices and Edges
    private StringVertex vR, vA, vB, vC, vD, vX, vY, vZ, vP;
    private StringEdge eRA, eRB, eAC, eAD, eXY, eXZ, ePR;

    // Suppliers
    private Supplier<Graph<StringVertex, StringEdge>> graphSupplier;
    private SubtreeReplacer.EdgeFactory<StringVertex, StringEdge> edgeFactory;

    @Before
    public void setUp() {
        vR = new StringVertex("R");
        vA = new StringVertex("A");
        vB = new StringVertex("B");
        vC = new StringVertex("C");
        vD = new StringVertex("D");
        vX = new StringVertex("X");
        vY = new StringVertex("Y");
        vZ = new StringVertex("Z");
        vP = new StringVertex("P");

        eRA = new StringEdge("R->A");
        eRB = new StringEdge("R->B");
        eAC = new StringEdge("A->C");
        eAD = new StringEdge("A->D");
        eXY = new StringEdge("X->Y");
        eXZ = new StringEdge("X->Z");
        ePR = new StringEdge("P->R");

        graphSupplier = () -> new DefaultDirectedGraph<>(StringEdge.class);
        // This edge factory assumes that if originalEdge is null (e.g. when replacing the root and there's no parent edge to reuse),
        // it creates a new label. Otherwise, it reuses the original edge's label for the new edge.
        edgeFactory = (source, target, originalEdge) ->
                new StringEdge(originalEdge != null ? originalEdge.getLabel() : source.getLabel() + "->" + target.getLabel() + "_newFromFactory");
    }
    ...
    public void replaceSubtree_nonMutating_nodeToReplaceIsRoot_replacesEntireGraph() {
        ...
        final Graph<StringVertex, StringEdge> originalGraph = newGraphBuilder()
                .addVertices(vR, vA).addEdge(vR, vA, eRA).build();

        // Replacement Tree: X -> Y (eXY)
        final Graph<StringVertex, StringEdge> replacementGraph = newGraphBuilder()
                .addVertices(vX, vY).addEdge(vX, vY, eXY).build();
        ...
        final Graph<StringVertex, StringEdge> expectedReturnedGraph = newGraphBuilder()
                .addVertices(vX, vY).addEdge(vX, vY, new StringEdge(eXY.getLabel())).build();

        // When
        final Graph<StringVertex, StringEdge> returnedGraph = SubtreeReplacer.replaceSubtreeWithTree(
                originalGraph, vR, replacementGraph, graphSupplier, edgeFactory);
    ...
    public void replaceSubtree_nonMutating_nodeToReplaceIsChild() {
        ...
        //              --eRB--> B
        final Graph<StringVertex, StringEdge> originalGraph = newGraphBuilder()
                .addVertices(vP, vR, vA, vB).addEdge(vP, vR, ePR).addEdge(vR, vA, eRA).addEdge(vR, vB, eRB).build();
    ...
    public void replaceSubtree_nonMutating_emptyReplacement_removesSubtree() {
        ...
        final Graph<StringVertex, StringEdge> originalGraph = newGraphBuilder()
                .addVertices(vP, vR, vA).addEdge(vP, vR, ePR).addEdge(vR, vA, eRA).build();

        // Replacement Tree: (empty)
        final Graph<StringVertex, StringEdge> emptyReplacementGraph = newGraphBuilder().build();

        // Expected Returned Tree:
        //   P
        // (R and A and edges ePR, eRA are removed; P remains, R's subtree is pruned)
        final Graph<StringVertex, StringEdge> expectedReturnedGraph = newGraphBuilder().addVertex(vP).build();
        // When
        final Graph<StringVertex, StringEdge> returnedGraph = SubtreeReplacer.replaceSubtreeWithTree(
                originalGraph, vR, emptyReplacementGraph, graphSupplier, edgeFactory);
        // Then
        assertGraphsAreStructurallyEqual(expectedReturnedGraph, returnedGraph);
        assertThat("Returned graph with empty replacement should only contain parent P.",
                   returnedGraph.vertexSet(), contains(vP));
        assertThat("Returned graph with empty replacement should have no edges.",
                   returnedGraph.edgeSet(), is(empty()));
        assertThat(returnedGraph, is(not(sameInstance(originalGraph))));
    }
    ...
    public void replaceSubtree_nonMutating_nodeToReplaceIsLeaf() {
        ...
        final Graph<StringVertex, StringEdge> originalGraph = newGraphBuilder()
                .addVertices(vP, vR).addEdge(vP, vR, ePR).build();

        // Replacement Tree: X (single node, root X)
        final Graph<StringVertex, StringEdge> replacementGraph = newGraphBuilder().addVertex(vX).build();

        // Expected Returned Tree: P --(label from ePR)--> X
        final Graph<StringVertex, StringEdge> expectedReturnedGraph = newGraphBuilder()
                .addVertices(vP, vX).addEdge(vP, vX, new StringEdge(ePR.getLabel())).build();
        // When
        final Graph<StringVertex, StringEdge> returnedGraph = SubtreeReplacer.replaceSubtreeWithTree(
                originalGraph, vR, replacementGraph, graphSupplier, edgeFactory);
        // Then
    ...
    public void replaceSubtree_nonMutating_nodeToReplaceNotInGraph_returnsOriginalGraphInstance() {
        ...
        // When
        final Graph<StringVertex, StringEdge> returnedGraph = SubtreeReplacer.replaceSubtreeWithTree(
                originalGraph, nonExistentNode, replacementGraph, graphSupplier, edgeFactory);
        // Then
        assertThat("Returned graph should be the same instance as original when node not found.",
                   returnedGraph, is(sameInstance(originalGraph)));
    ...
    public void replaceSubtree_nonMutating_replacementGraphIsEmptyAndNodeToReplaceIsRoot_resultsInEmptyGraph() {
        ...
        final Graph<StringVertex, StringEdge> expectedReturnedGraph = newGraphBuilder().build();
        // When
        final Graph<StringVertex, StringEdge> returnedGraph = SubtreeReplacer.replaceSubtreeWithTree(
                originalGraph, vR, emptyReplacementGraph, graphSupplier, edgeFactory);
        // Then
    ...
    public void replaceSubtree_nonMutating_originalGraphIsEmpty_nodeToReplaceNotFound() {
        // Given
        // Original Tree: (empty)
        final Graph<StringVertex, StringEdge> originalGraph = newGraphBuilder().build();
        final Graph<StringVertex, StringEdge> originalGraphSnapshotForExpected = newGraphBuilder().build();
    ...
    // Consider if a test for "replacement graph is not a single rooted tree" is needed,
    // depending on how SubtreeReplacer should behave with such input.
    // If it should throw an IllegalArgumentException, a test for that would be good.
    // If it's guaranteed by the caller that replacementGraph is always a single tree,
    // then the complex multi-root test is indeed unnecessary.
}